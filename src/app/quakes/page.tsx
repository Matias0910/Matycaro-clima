    ...
    <main className="min-h-screen bg-slate-950 text-white p-6 max-w-md mx-auto flex flex-col justify-between">
      <div>
        ...
        <header className="flex justify-between items-center mb-6">
          ...
          <button 
            onClick={fetchQuakes}
            className="p-2 bg-slate-900 rounded-full hover:bg-slate-800 transition text-blue-400 border border-slate-800"
            title="Actualizar"
          >
        ...
        <div className="space-y-3 mb-6">
          {loading ? (
            ...
            quakes.map((q) => {
              ...
              return (
                <div key={q.id} className={`p-4 rounded-2xl border flex justify-between items-center ${inArg ? 'bg-slate-900 border-amber-500/50 shadow-lg shadow-amber-500/5' : 'bg-slate-900/60 border-slate-800'}`}>
                  <div>
                  ...
                  <div className="text-right shrink-0 ml-2">
                    <Activity size={18} className={`ml-auto mb-1 ${inArg ? 'text-amber-400' : 'text-slate-500'}`} />
